...
CollectionProvider.prototype.findByObject = function(object, callback) {
    this.getCollection(function(error, collection) {
      if( error ) callback(error)
      else {
        collection.find(object).toArray(function(error, results) {
          if( error ) callback(error)
          else callback(null, results)
        });
      }
...
CollectionProvider.prototype.updateByInternalId = function(id, data, callback) {
    ...
    this.update({_id: new ObjectID(id)}, data, callback);
};
